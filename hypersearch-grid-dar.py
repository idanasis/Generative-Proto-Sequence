...
@dataclass
class Config:
    file_to_replace: str = "sbatch_dar"
    original_command_pattern: str = "poetry run python dar_simplegrid_levels.py"
    ...
    param_grid: dict[str, Any] = field(default_factory=lambda: {
        ...
        'obstacle_map': ["16x16_room_v1"],
        'train_dataset_size': [100],
        'train_eval_dataset_size': [100],
        'val_dataset_size': [100],
        'test_dataset_size': [1000],
        'dqn_linear_layers': ["512 128 32"],
        'dqn_linear_layers_activation_function': ["leaky_relu"],
        'dar_r_l': ["1 5 10"]
    })
...
class ExperimentConfig:
    ...
    dqn_linear_layers: str
    dqn_linear_layers_activation_function: str
    dar_r_l: str
...
def transform_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    # Join list fields
    list_fields = ['dqn_linear_layers', 'dar_r_l']
...
def get_finished_runs(api: wandb.Api, wandb_project_name: str) -> tuple[list[RunData], list[Run]]:
    ...
    for run in api.runs(wandb_project_name):
        if run.state == "finished" or run.state == "running":
            ...
            field_defaults = {
                'dar_r_l': '1 5 10',  # Default value based on Config.param_grid
                'partial_observability_strategy': 'FULL',
                'view_radius': 3,
                'is_slippery': False,
                'slippery_prob': 1 / 3,
...
def hypersearch(cfg: Config):
    ...
    if cfg.top_k > 0:
        ATTR_MATCHING_L: list[str] = ['train_dataset_size', 'total_timesteps', 'obstacle_map', 'dqn_linear_layers',
                                      'buffer_size',
                                      'max_level', 'start_level', 'reward_strategy', 'dar_r_l', 'seed']
